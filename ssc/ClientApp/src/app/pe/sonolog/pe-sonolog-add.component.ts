...
@Component({
	selector: 'app-sonolog-add',
	templateUrl: './pe-sonolog-add.component.html',
	styleUrls: ['./pe-sonolog.scss']
})

export class PeSonologAddComponent {
	...
	onSubmit() { 
		...
		this.sonologForm.disable();
	}

	get f() { return this.sonologForm.controls; }
	...
	ngOnInit() { 
		...
		this.sonologForm = this.formBuilder.group({
	...
	listSonolog() {
		this.router.navigate(['pe', 'sonolog', 'list']);
	}

	canDeactivate(): Observable<boolean> | boolean {
		if (this.sonologForm.pristine) {
			return true;
		}
	...
	onUpload() {
		...
		this.http.post('/api/pe/sonolog/UploadFiles', fd, {
			reportProgress: true,
			observe: 'events'
		})
		.subscribe(event => {
	...
	loadData() {
		...
		this.http.get<any>('/api/pe/sonolog/Tmp', httpOption).subscribe(res => {
	...
	saveData() {
		this.isSaving = true;
		this.http.get<any>('/api/pe/sonolog/SaveData', {params: {_id: this.tmp_id}}).subscribe(res => {
			this.isSaving = false;
	...
	unloadNotification($event: any) {
		return this.sonologForm.pristine;
	}
