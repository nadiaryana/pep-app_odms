...
@Component({
	selector: 'app-actual-add',
	templateUrl: './pe-actual-add.component.html',
	// styleUrls: ['./pe-actual-add.component.scss']
})

export class PeActualAddComponent {
	...
	displayedColumns: string[] = ["info","date","total_opr","sgt_mgs", "sbr_nsop", "bd"];
  	headerColumns1: string[] = ["info","date","total_opr","sgt_mgs", "sbr_nsop", "bd"];
  ...
  removeOpsogForm(index) {
	...
	onUpload() {
		...
		this.http.post('/api/pe/actual/UploadFilesActual', fd, {
			reportProgress: true,
			observe: 'events'
		})
		.subscribe(event => {
  ...
  // this.http.post('/api/pe/daily/UploadFiles', this.opsogForm.value, {});
   ...
   this.stepper.next();
	...
	saveData() {
		...
		this.http.get<any>(`/api/pe/actual/SaveDataActual?tmp_id=${this.tmp_id}`)
			.subscribe(res => {
			this.isSaving = false;
