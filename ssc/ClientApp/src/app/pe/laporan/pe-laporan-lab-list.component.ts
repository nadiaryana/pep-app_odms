...
import { PeLaporanLab } from './pe-laporan-lab';

@Component({
  selector: 'app-pe-laporan-lab-list',
  templateUrl: './pe-laporan-lab-list.component.html',
  styleUrls: ['./pe-laporan-lab-list.component.scss']
})
export class PeLaporanLabListComponent implements OnInit {

    displayedColumns: string[] = [
      "select","nomor", "date","well","sed","water","sludge","total",
      "api","sg","density_obs","density_dua", "pp", "temperature", "visc", "cl", "rw", "keterangan"];
    headerColumns1: string[] = ["select","nomor", "date","well","bsnw","pemeriksaan_lab"];
    headerColumns2: string[] = ["sed","water","sludge","total","api","sg","density_obs","density_dua", "pp", "temperature", "visc", "cl", "rw", "keterangan"];
    ...
    sedFilter = new FormControl('');
    waterFilter = new FormControl('');
    sludgeFilter = new FormControl('');
    totalFilter = new FormControl('');
    apiFilter = new FormControl('');
    sgFilter = new FormControl('');
    density_obsFilter = new FormControl('');
    density_duaFilter = new FormControl('');
    ppFilter = new FormControl('');
    temperatureFilter = new FormControl('');
    viscFilter = new FormControl('');
    clFilter = new FormControl('');
    rwFilter = new FormControl('');
    ...
    api_xSelected = [];
    sg_xSelected = [];
    density_obs_xSelected = [];
    density_dua_xSelected = [];
    pp_xSelected = [];
    temperature_xSelected = [];
    visc_xSelected = [];
    cl_xSelected = [];
    rw_xSelected = [];
  ...
  constructor(
    ...
    // private pe_sonologService: PeSonologService,
  ...
  ngOnInit() {
    ...
    this.titleService.titleSource.next({
          title: "Meassurement Result",
          icon: "data_exploration",
          breadcrumbs: [
            {label: 'Petroleum Engineering', routerLink: ''}, 
            {label: 'Meassurement Result', routerLink: ''}
          ]}
        );
        ...
        this.listSubscription = merge(
          ...
          this.sedFilter.valueChanges.pipe(debounceTime(300)),
          this.waterFilter.valueChanges.pipe(debounceTime(300)),
          this.sludgeFilter.valueChanges.pipe(debounceTime(300)),
          this.totalFilter.valueChanges.pipe(debounceTime(300)),
          this.apiFilter.valueChanges.pipe(debounceTime(300)),
          this.sgFilter.valueChanges.pipe(debounceTime(300)),
          this.density_obsFilter.valueChanges.pipe(debounceTime(300)),
          this.density_duaFilter.valueChanges.pipe(debounceTime(300)),
          this.ppFilter.valueChanges.pipe(debounceTime(300)),
          this.temperatureFilter.valueChanges.pipe(debounceTime(300)),
          this.viscFilter.valueChanges.pipe(debounceTime(300)),
          this.clFilter.valueChanges.pipe(debounceTime(300)),
          this.rwFilter.valueChanges.pipe(debounceTime(300)),
      ...
      getColumnFilter() {
        ...
        if(this.sed_xSelected.length) columnfilter["sed"] = this.sed_xSelected;
        if(this.water_xSelected.length) columnfilter["water"] = this.water_xSelected;
        if(this.sludge_xSelected.length) columnfilter["sludge"] = this.sludge_xSelected;
        if(this.total_xSelected.length) columnfilter["total"] = this.total_xSelected;
        if(this.api_xSelected.length) columnfilter["api"] = this.api_xSelected;
        if(this.sg_xSelected.length) columnfilter["sg"] = this.sg_xSelected;
        if(this.density_obs_xSelected.length) columnfilter["density_obs"] = this.density_obs_xSelected;
        if(this.density_dua_xSelected.length) columnfilter["density_dua"] = this.density_dua_xSelected;
        if(this.pp_xSelected.length) columnfilter["pp"] = this.pp_xSelected;
        if(this.temperature_xSelected.length) columnfilter["temperature"] = this.temperature_xSelected;
        if(this.visc_xSelected.length) columnfilter["visc"] = this.visc_xSelected;
        if(this.cl_xSelected.length) columnfilter["cl"] = this.cl_xSelected;
        if(this.rw_xSelected.length) columnfilter["rw"] = this.rw_xSelected;
      ...
      deleteSelected() {
        ...
        const dialogRef = this.dialog.open(PeLaporanLabDeleteDialogComponent, {
          width: '250px',
        ...
        dialogRef.afterClosed().subscribe(result => {
          if(result) {
            this.isLoadingResults = true; 
            this.snackbarService.status.next(new SnackbarApi(false));
            this.http.delete<any>('/api/pe/laporan', {
              headers: new HttpHeaders({
                'Content-Type': 'application/json'
              }),
              params: {
    ...
    export interface PeLaporanLabApi {
      items: PeLaporanLab[];
      total_count: number;
    }
    ...
    export class ExampleHttpDao {
      ...
      getRepoIssues(sort: string, order: string, page: number, pagesize: number = 50, filter: string, columnfilter: object, mode: string = "", httpOption: object = {}): Observable<PeLaporanLabApi> {
        ...
        return this.http.get<PeLaporanLabApi>('/api/pe/laporan', httpOption);
      }
    }
    
    @Component({
      selector: 'app-laporan-delete-dialog',
      ...
      styleUrls: ['./pe-laporan-lab.scss']
    })
    export class PeLaporanLabDeleteDialogComponent {
    
      constructor(
        public dialogRef: MatDialogRef<PeLaporanLabDeleteDialogComponent>,
