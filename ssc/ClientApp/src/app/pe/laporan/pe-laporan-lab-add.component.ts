...
@Component({
  selector: 'app-pe-laporan-lab-add',
  templateUrl: './pe-laporan-lab-add.component.html',
  styleUrls: ['./pe-laporan-lab-add.component.scss']
})
export class PeLaporanLabAddComponent {
  @Input() locations: Location[];
    ...
    displayedColumns: string[] = [
      "info","nomor", "date","well","sed","water","sludge","total",
      "api","sg","density_obs","density_dua", "pp", "temperature", "visc", "cl", "rw", "keterangan"];
    headerColumns1: string[] = ["info","nomor", "date","well","bsnw","pemeriksaan_lab"];
  ...
  ngOnInit() {
    this.titleService.titleSource.next({
              title: "Add Measurement Result",
              icon : "add",
            breadcrumbs: [
              {label: 'Petroleum Engineering', routerLink: ''}, 
              {label: 'Laporan', routerLink: 'pe/laporan'},
              {label: 'Add', routerLink: ''}, 
            ]}
          );
    
        this.laporanForm = this.formBuilder.group({
          //sonolog_id: ['', Validators.required],
          location_id: [''],
          is_anchor: [''],
        });
      ...
      listSonolog() {
        this.router.navigate(['pe', 'laporan', 'list']);
      }
    
      canDeactivate(): Observable<boolean> | boolean {
        if (this.laporanForm.pristine) {
          return true;
        }
      ...
      onUpload() {
        ...
        this.http.post('/api/pe/laporan/UploadFiles', fd, {
          reportProgress: true,
          observe: 'events'
        })
        .subscribe(event => {
      ...
      loadData() {
        ...
        this.http.get<any>('/api/pe/laporan/Tmp', httpOption).subscribe(res => {
      ...
      saveData() {
        this.isSaving = true;
        this.http.get<any>('/api/pe/laporan/SaveData', {params: {_id: this.tmp_id}}).subscribe(res => {
          this.isSaving = false;
      ...
      unloadNotification($event: any) {
        return this.laporanForm.pristine;
      } 
}
